...
CRadioServiceAppDlg::cur_time CRadioServiceAppDlg::get_cur_time(){
	...
	GetLocalTime(&st);  // ѕолучаем локальное врем€
...
BOOL CRadioServiceAppDlg::OnInitDialog()
{
	...
	//m_TotalSize = CSize(800, 600); // Ќачальный размер виртуального пространства
	//UpdateScrollBars();
	//calculate_fft(this);
...
void CRadioServiceAppDlg::OnPaint()
{
	...
	{
		//CDialogEx::OnPaint();
		//берем отрисовку всего фона на себ€, остальный контролы кнопки и тд сами себ€ рисуют
